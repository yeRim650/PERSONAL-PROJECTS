...
public class Interceptor extends FlyUnit implements IProtossUnit{
	private LandAttack landAttack;
	private AirAttack airAttack;
	private int shield;
	
	public Interceptor(User user, Point point){
		super(user, point, 40, 40);
		setLandAttack(new LandAttack(6));
		setAirAttack(new AirAttack(6));
		setShield(40);
	}
	public Interceptor(User user, Point point, int hp, int maxHp, LandAttack landAttack, AirAttack ariAttack, int shield){
