...
public class ImageNumJPanel extends JPanel{
	public static final String NUMS = "num";
	...
	public ImageNumJPanel(MyNums nums, ArrayList<Integer> myLists, Integer ranks){
		...
		for(int i = 0; i<6;i++){
			lbls[i] = new JLabel();
			lbls[i].setIcon(setSize(PATH_WHITE+NUMS+nums.getNums()[i]+EXE));
			for(Integer list : myLists){
				if(nums.getNums()[i]==list){
					lbls[i].setIcon(setSize(PATH_COLOR+NUMS+nums.getNums()[i]+EXE));
				}
			}
			add(lbls[i]);
		}
	...
	public ImageNumJPanel(LottoNums loNums){
		...
		for(int i = 0; i<7;i++){
			...
			lbls[i].setIcon(setSize(PATH_COLOR+NUMS+loNums.getNums()[i]+EXE));
			add(lbls[i]);
		}
		setBackground(Color.WHITE);
	}
